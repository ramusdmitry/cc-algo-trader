...
# from bybit_mapping import bybit_tif_mapping, bybit_order_type_mapping, bybit_order_status_mapping, \
...
class Bybit:
    ...
    def __init_client(self):
        ...
        self.is_unified_account = (
            True if retry(lambda: self.client.get_account_info())["unifiedMarginStatus"] > 2 else False
        )
        ...
        if self.quote_rounding is None or self.asset_rounding is None:
            ...
            self.asset_rounding = (
                abs(Decimal(str(market[0]["lotSizeFilter"][base_precision])).as_tuple().exponent)
    ...
    def get_position(self, symbol=None, force_api_call=True):
        ...
        def get_position_api_call():
            positions = retry(lambda: self.client.get_positions(symbol=symbol, category=self.category))["list"]

            self.position = [p for p in positions if p["symbol"] == symbol]

            if self.position is None or len(self.position) == 0:
                self.position = [{"avgPrice": 0, "size": 0, "liqPrice": 0, "side": None}]
                return None
    ...
    def get_profit(self, close=None, avg_entry_price=None, position_size=None, commission=None):
        ...
        if avg_entry_price is None:
            avg_entry_price = self.entry_price if self.entry_price is not None else self.get_position_avg_price()
        ...
        profit = round(
            abs(position_size) * close_rate * (1 if self.qty_in_usdt else avg_entry_price),
            self.quote_rounding,
        )
    ...
    def get_orderbook(self):
        ...
        """
        self.__init_client()
        ob = retry(lambda: self.client.get_orderbook(symbol=self.pair, category=self.category))
    ...
    def __new_order(
        ...
        Args:
            ...
            trailing_stop (float, optional): Trailing stop price for a trailing stop-limit order.
            Default is 0. (use set_trading_stop() once in a position)
            activationPrice (float, optional): Activation price for a stop-limit order with activation.
            Default is 0. (use set_trading_stop() once in a position)
    ...
    def entry(
        ...
        Args:
            ...
            limit_chase_init_delay (float, optional): Limit Chase Init Delay.
            Initial delay for limit order chasing (used when post_only is True and limit_chase_interval > 0).
        ...
        if chaser:
            ...
            class Chaser:
                ...
                def stats(self, status="FILLED"):
                    ...
                    slippage = (avg_price - start_price if self.long else start_price - avg_price) / start_price
                    logger.info("--------------------------------------")
            ...
            return Chaser(id, long, qty, limit, stop, post_only, reduce_only, callback, trigger_by)
    ...
    def get_open_orders(
        ...
        Returns:
            list or dict or None: List of open orders if not separated,
            a dictionary containing separate keys for active and conditional orders if `separate=True`,
        ...
        user_id = "orderLinkId"
        active_orders = retry(lambda: self.client.get_open_orders(symbol=self.pair, category=self.category))["list"]
        conditional_orders = (
            retry(
                lambda: self.client.get_open_orders(symbol=self.pair, category=self.category, orderFiler="StopOrder")
            )["list"]
        ...
        if id is not None:
            if separate:
                orders = {
                    ...
                    "conditional_orders": [
                        order for order in orders["conditional_orders"] if order[user_id].startswith(id)
                    ],
                }
            else:
    ...
    def eval_exit(self):
        ...
        if self.get_exit_order()["trail_offset"] > 0 and self.get_trail_price() > 0:
            if (
                    ...
                    and self.get_market_price() - self.get_exit_order()["trail_offset"] < self.get_trail_price()
            ):
    ...
    def fetch_ohlcv(self, bin_size, start_time, end_time, demo=False):
        self.__init_client()
                ...
                source_to_object_list.append(
                    {
                        "timestamp": (
                                datetime.fromtimestamp(
                                    int(timestamp / 1000 if len(str(timestamp)) == 13 else timestamp))
        ...
        for t in timeframes_to_update:
            ...
            if self.call_strat_on_start:
                if (
                        ...
                        and self.timeframe_info[t]["last_action_time"] == re_sample_data.iloc[-1].name
                ):
                    continue
            else:
    ...
    def __on_update_position(self, action, position):
        ...
        if is_update_pos_size:
            ...
            pos_ = self.position[0]
            pos = position[0]
            message = (
                f"Updated Position\n"
                f"Price: {pos_['avgPrice']} => {pos['entryPrice']}\n"
                f"Size: {pos_['size'] * (-1 if pos_['side'] == 'Sell' else 1)} => {pos['size'] * (-1 if pos['side'] == 'Sell' else 1)}\n"
                f"Liquidation Price: {parseFloat(pos_['liqPrice'])} => {parseFloat(pos['liqPrice'])}\n"
        ...
        if "snapshot" in action:
            ...
            for side, entries in bookticker.items():
                for entry in entries:
                    ...
                    price_level_exists = entry[0] in [level[0] for level in self.bookticker[side]]
                    if not price_level_exists:
                        self.bookticker[side].append(entry)
                        continue

                    qty_changed = entry[1] != next(level[1] for level in self.bookticker[side] if level[0] == entry[0])
